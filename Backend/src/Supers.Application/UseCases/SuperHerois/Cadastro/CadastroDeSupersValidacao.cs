...
namespace Supers.Application.UseCases.SuperHerois.Cadastro
{
    public class CadastroDeSupersValidacao : AbstractValidator<CadastroSuperRequest>
    {
        public CadastroDeSupersValidacao()
        {
            ...
            RuleFor(user => user.Nome).Matches(@"^[a-zA-ZÀ-ÿ\s]+$").WithMessage(Mensagens.NOME_CARACTERES_ESPECIAIS);
            RuleFor(user => user.Nome).MaximumLength(25).WithMessage(Mensagens.NOME_TAMANHO_MAXIMO);
            RuleFor(user => user.NomeHeroi).NotEmpty().WithMessage(Mensagens.NOME_HEROI_VAZIO);
            RuleFor(user => user.NomeHeroi).Matches(@"^[a-zA-ZÀ-ÿ\s]+$").WithMessage(Mensagens.NOME_CARACTERES_ESPECIAIS);
            RuleFor(user => user.NomeHeroi).MaximumLength(25).WithMessage(Mensagens.NOME_HEROI_TAMANHO_MAXIMO);
            RuleFor(user => user.SuperPoderes).NotEmpty().WithMessage(Mensagens.SUPER_PODERES_VAZIO);
            RuleFor(user => user.DataDeDascimento).NotEmpty().WithMessage(Mensagens.DATA_NASCIMENTO_VAZIO);
